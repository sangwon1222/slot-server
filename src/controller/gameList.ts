...
import { NoError, SQLError } from "../util/errors";
...
export async function getGameList(params: { name: string }) {
  try {
    ...
    const result = err
      ? err
      : { ...NoError, ok: true, data: searchQuery.results };
    return result;
  } catch (e) {
...
export async function createGame({ name, turnCount, score }: any) {
  ...
  try {
    ...
    if (createGameQuery.err?.code == "ER_TABLE_EXISTS_ERROR")
      return { ok: false, msg: "중복된 게임명입니다." };
    if (createGameQuery.err)
      return { ok: false, msg: createGameQuery.err.message };

    const insert = await DB.query(
      `INSERT INTO gameList (idx, name, turnCount, score) VALUES (NULL,?,?,?)`,
      [name, turnCount, score]
    );
...
export async function getGame({ name, groupIdx }: any) {
  try {
    ...
    const queryString =
      groupIdx != undefined
        ? `SELECT * FROM ${name.toUpperCase()} WHERE groupIdx = '${groupIdx}' ORDER BY idx ASC`
        : `SELECT * FROM ${name.toUpperCase()} ORDER BY idx ASC`;
...
export async function deleteGame(name: string) {
  try {
    ...
    if (
      ...
      const result = err
        ? err
        : { ...NoError, ok: true, data: deleteGameQuery.results };
      return result;
    }
  } catch (e) {
    return { ok: false, data: [], msg: e };
  ...
  `;
    ...
    for (let i = 0; i < rowData.length; i++) {
      ...
      if (i == rowData.length - 1)
        return { ...NoError, ok: true, data: insertQuery.results };
    }
  } catch (e) {
    return { ok: false, data: [], msg: e };
  }
...
export async function deletePrize(name: string) {
  try {
    ...
    const result = err
      ? { ok: false, err, msg: "PRIZE 삭제 실패" }
      : { ...NoError, ok: true, data: deleteQuery.results };
...
export async function getScore(score: number) {
  try {
    const stringQuery = score
      ? `SELECT * FROM SCORELIST WHERE score = ?;`
      : `SELECT * FROM SCORELIST;`;
    const getQuery = await DB.query(stringQuery, [score]);
    const err = checkQueryError(getQuery);
    const result = err
      ? { ok: false, err, msg: "GET SCORE 실패" }
      : { ...NoError, ok: true, data: getQuery.results };
...
export async function addScore(
  ...
  try {
    ...
    const err = checkQueryError(insertQuery);
    const result = err
      ? { ok: false, err, msg: "ADD SCORE 실패" }
      : { ...NoError, ok: true, data: insertQuery.results };
    return result;
  } catch (e) {
    return { ok: false, data: [], msg: e };
  }
