...
export async function getUserTurnData(
  userID: string
  ...
  try {
    ...
    const game = await DB.query(
      `SELECT * FROM ${gameTable} WHERE groupIdx=? AND idx=?;`,
      [currentGroupIndex, prizeTurn]
    );
    ...
    return {
      ok: true,
      data: {
        Ycode: turnCount == turnInPrize + 1 ? Ycode : "-",
        Pcode: turnCount == turnInPrize + 1 ? Pcode : "-",
        score: turnCount == turnInPrize + 1 ? score : 0,
        turnCount,
      },
    };
  } catch (e) {
...
export async function updateReelData(
  userID: string
): Promise<{ ok: boolean; data: any }> {
  try {
    ...
    const game = await DB.query(`SELECT * FROM ${gameTable} WHERE idx=?;`, [
      prizeTurn,
    ]);
    if (!game.results?.length) return { ok: false, data: {} };
    const { groupIdx, groupCount, turnCount, Ycode, Pcode, score, outScore } =
      game.results[0];
    ...
    if (turnInPrize + 1 == turnCount) {
      await DB.query(
        ...
        [+groupIdx, +user.data.prizeTurn + 1, 0, userID]
      );
      return {
        ok: true,
        data: {
      ...
      await DB.query(
        `UPDATE user SET currentGroupIndex=?, prizeTurn=?, turnInPrize=? WHERE userID=?;`,
        [+groupIdx, +user.data.prizeTurn, +turnInPrize + 1, userID]
      );
      return {
        ok: true,
        data: {
...
export async function getUserList(
  ...
  try {
    const queryString = userID
      ? `SELECT currentGroupIndex, gameTable, prizeTurn, turnInPrize FROM user WHERE userID='${userID}' ORDER BY idx;`
...
export async function remove(userID: string) {
  try {
    ...
    if (deleteUser.results.length == 0)
...
export async function login(
  ...
  try {
    ...
    const err_select = checkQueryError(select_);
    if (err_select)
      return { ok: false, msg: err_select.msg as string, data: "" };
...
export async function registGame(userID: string, gameName: string) {
  try {
    const result = await DB.query(
      `UPDATE user SET gameTable=? WHERE userID=?;`,
      [gameName, userID]
    );
    const err = checkQueryError(result);
    if (err) return err;
